...
public class UserServiceDetailImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        return userRepository.findByUserNameIgnoreCase(username)
                .<UserDetails>map(UserDetailImpl::new)

                // 🔹 2. ADMIN
                .or(() -> adminRepository.findByEmailIgnoreCase(username)
                        .map(UserDetailImpl::new))

                // 🔹 3. COLLECTOR
                .or(() -> collectorRepository.findByCollectorIdIgnoreCase(username)
                        .map(UserDetailImpl::new))

                // 🔴 NOT FOUND
                .orElseThrow(() ->
                        new UsernameNotFoundException("User/Admin/Collector not found: " + username)
                );
    }
}
